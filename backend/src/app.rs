...
impl LittleLocatorApp {
  ...
  pub fn paint_location(&mut self, ui: &mut egui::Ui) -> egui::Response {
    ...
    let tag_txr = ui.ctx().load_texture(
      "tag",
      egui::ImageData::Color(Arc::new(load_image_from_path(&std::path::PathBuf::from("../frontend/assets/position.png")).unwrap())),
      Default::default(),
    );
